...
for ii = 1:size(Filenames,1)
    ...
    try
        ...
        if size(result.speed,1)>0
            result.filt_speed_gauss = zeros(15750,1);
    ...
    %     figure; hold on; plot(result.filt_speed_gauss);
    %     plot(result.filt_speed_gauss+0.5,'r');
    %     plot(result.filt_speed_gauss+1,'k');
    %       plot(result.filt_speed_gauss+1.5,'g');
    ...
    if isnan(freeze_periods(ii,3))==0
        if (freeze_periods(ii,3)-startanalysisFrame(ii))>=(30*75) && (freeze_periods(ii,4)-startanalysisFrame(ii))<15750 && (freeze_periods(ii,4)-freeze_periods(ii,3))>=(0.5*75)
            ...
            %             result.filt_speed_gauss((30*75):(freeze_periods(ii,4)-startanalysisFrame(ii))) = 1;
        elseif (freeze_periods(ii,3)-startanalysisFrame(ii))>=(30*75) && (freeze_periods(ii,4)-startanalysisFrame(ii))>15750 && (freeze_periods(ii,4)-freeze_periods(ii,3))>=(0.5*75)
    ...
    if isnan(freeze_periods(ii,5))==0
        if (freeze_periods(ii,5)-startanalysisFrame(ii))>=(30*75) && (freeze_periods(ii,6)-startanalysisFrame(ii))<15750 && (freeze_periods(ii,6)-freeze_periods(ii,5))>=(0.5*75)
            ...
            %             result.filt_speed_gauss((30*75):(freeze_periods(ii,6)-startanalysisFrame(ii))) = 1;
        elseif (freeze_periods(ii,5)-startanalysisFrame(ii))>=(30*75) && (freeze_periods(ii,6)-startanalysisFrame(ii))>15750 && (freeze_periods(ii,6)-freeze_periods(ii,5))>=(0.5*75)
    ...
    if isnan(escape_periods(ii,1))==0
        if (escape_periods(ii,1)-startanalysisFrame(ii))>=(30*75) && (escape_periods(ii,1)-startanalysisFrame(ii))<(40*75) && (escape_periods(ii,2)-startanalysisFrame(ii))<15750
            result.filt_speed_gauss((escape_periods(ii,1)-startanalysisFrame(ii)):(escape_periods(ii,2)-startanalysisFrame(ii))) = 2;
        elseif (escape_periods(ii,1)-startanalysisFrame(ii))<(30*75)  && (escape_periods(ii,1)-startanalysisFrame(ii))<(40*75) && (escape_periods(ii,2)-startanalysisFrame(ii))<15750
    ...
    % convert escape #2
    if isnan(escape_periods(ii,3))==0
        if (escape_periods(ii,3)-startanalysisFrame(ii))>=(30*75)  && (escape_periods(ii,4)-startanalysisFrame(ii))<15750
            result.filt_speed_gauss((escape_periods(ii,3)-startanalysisFrame(ii)):(escape_periods(ii,4)-startanalysisFrame(ii))) = 2;
            %         elseif (escape_periods(ii,3)-startanalysisFrame(ii))<(30*75)  && (escape_periods(ii,4)-startanalysisFrame(ii))<15750
            %             result.filt_speed_gauss((30*75):(escape_periods(ii,4)-startanalysisFrame(ii))) = 2;
        elseif (escape_periods(ii,3)-startanalysisFrame(ii))>=(30*75)  && (escape_periods(ii,4)-startanalysisFrame(ii))>15750
...
for ii = 1:size(speed,1)
    ...
    if sum(isnan(speed(ii,:))==0)>0
        ...
        maxnorm_speed_all=[maxnorm_speed_all; speed(ii,(25*75):(45*75))];
    end
end
...
%Make sort index
%1. sum of NaN values (refuge)
%2. latency to escape
%3. duration of freeze
sortcolumns = [];
for ii = 1:size(speed,1)

    % 1
    if sum(isnan(speed(ii,(30*75):(40*75))))>0
        sortcolumns(ii,1) = sum(isnan(speed(ii,(30*75):(40*75))));
    else
        sortcolumns(ii,1)=0;
    end

    % 2
    if sum(speed(ii,(30*75):(40*75))==2)>0
        sortcolumns(ii,2) = find(speed(ii,(30*75):(40*75))==2,1,'first');
    else
        sortcolumns(ii,2) = NaN;
    end

    % 3
    if sum(speed(ii,(30*75):(40*75))==1)>0
        sortcolumns(ii,3) = sum(speed(ii,(30*75):(40*75))==1);
    else
        sortcolumns(ii,3) = 0;
    end
end
[~, sortidx] = sortrows(sortcolumns,[1 2 3],{'ascend' 'descend' 'ascend'});

% %Make sort index
% %1. latency to refuge (NaN)
% %2. latency to escape
% %3. latency to freeze
% %4. Dutation of freeze
% sortcolumns = [];
% for ii = 1:size(speed,1)
%
%     % 1
%     if sum(isnan(speed(ii,(30*75):(40*75))))>0
% sortcolumns(ii,1) = find(isnan(speed(ii,(30*75):(40*75))),1,'first');
%     else
...
% %     % 3
% %         if sum(speed(ii,(30*75):(40*75))==1)>0
% % sortcolumns(ii,3) = find(speed(ii,(30*75):(40*75))==1,1,'first');
% %     else
% %         sortcolumns(ii,3) = NaN;
% %         end
%     % 4
%         if sum(speed(ii,(30*75):(40*75))==1)>0
% sortcolumns(ii,3) = sum(speed(ii,(30*75):(40*75))==1);
...
% end
%
% [~, sortidx] = sortrows(sortcolumns,[2 1 3],{'descend' 'descend' 'descend'});
...
if analyse==1
    ...
    figname = 'Sweep_WT_classificationBox.fig';
    saveas(gcf,figname,'fig');
    figname = 'Sweep_WT_classificationBox.pdf';
    orient(gcf,'portrait')
    print('-dpdf','-painters','-loose',figname);
elseif analyse==2
    sortedTrials(1:size(sortidx,1),2) = [sortidx];
    % loadedTrials(1:size(sortidx,1),2) = [random_trials];

    figname = 'Sweep_enuc_classificationBox.fig';
    saveas(gcf,figname,'fig');
    figname = 'Sweep_enuc_classificationBox.pdf';
    orient(gcf,'portrait')
    print('-dpdf','-painters','-loose',figname);
elseif analyse==3
    sortedTrials(1:size(sortidx,1),3) = [sortidx];
    % loadedTrials(1:size(sortidx,1),3) = [random_trials];

    figname = 'Loom_WT_classificationBox.fig';
    saveas(gcf,figname,'fig');
    figname = 'Loom_WT_classificationBox.pdf';
    orient(gcf,'portrait')
    print('-dpdf','-painters','-loose',figname);
elseif analyse==4
    sortedTrials(1:size(sortidx,1),4) = [sortidx];
    % loadedTrials(1:size(sortidx,1),4) = [random_trial];

    figname = 'Loom_enuc_classificationBox.fig';
    saveas(gcf,figname,'fig');
    figname = 'Loom_enuc_classificationBox.pdf';
    orient(gcf,'portrait')
    print('-dpdf','-painters','-loose',figname);
end
