function [stimFA,stimHit,stimAUROC] = calcAUROC_EPSP (SAcounts,stimCounts)

countCounter = 0;
for i = min([SAcounts stimCounts]):0.001:max([SAcounts stimCounts])% if looking at EPSP amplitude for yoyo min(SAcounts):0.05:max(stimCounts)
